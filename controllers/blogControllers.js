...
const updateBlog = async (req, res) => {
  ...
  try {
    const blogToUpdate = await Blog.findOne({ slug: blogId });
    const slugit = slugify(newBlogTitle, { lower: true });
    const uniqueIdentifier = blogToUpdate._id.toString().slice(0, 10);
    const newSlug = `${slugit}-${uniqueIdentifier}`;

    if (!blogToUpdate) {
      return res.status(404).json({ error: "Blog not found" });
    }

    blogToUpdate.blogTitle = newBlogTitle;
    blogToUpdate.blogData = newBlogData;
    blogToUpdate.tags = newTags;
    blogToUpdate.cover_image_url = newCoverImageUrl;
...
const deleteBlog = async (req, res) => {
  ...
  try {
    // Find the blog using the slug to get its _id
    const blogToDelete = await Blog.findOne({ slug: id });
    ...
    const deleteCommentsResult = await Comments.deleteMany({ blog_id: deletedBlog._id });
